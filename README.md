...
1. 用户先用自然语言描述目标；需求通常允许模糊，但主体、用途、精确文字、参考图职责或动画动作存在关键分支时，AI 会集中提出最多四个高信息量问题。
2. 创作方向默认由 AI 自动匹配。AI 按“13 类交付物 → 19 个风格标签 → 10 个场景标签 → 22 个模板 → 相近 Case”筛选；用户也可以先锁定一个方向。
...
4. 对话 AI 判断需求足够完整后，系统会自动生成可追溯的最终提示词卡片，包含结构化 brief、方向/模板选择理由、参考图顺序和 2～6 个图片硬门槛；用户可以进入生图参数，也可以继续沟通。从已有成图继续时，该图片会在后续对话、最终提示词和生成参数间持续作为垫图。提示词、模式或参考图顺序变化后只会取消“最终提示词已就绪”标记；若请求明确使用某份最终提示词草稿，服务端仍会校验内容完全一致。
5. 生成分为草稿、精修、成品，服务端固定映射为 `low`、`medium`、`high`。前端传入其他 `quality` 值不能绕过映射；仍保持每张图一个独立请求，以保留逐张取消、结算、部分成功和动画帧顺序。
6. 结果可执行 AI 验收。验收先逐项检查硬门槛，任何缺项或失败都强制判定为“需要精修”；之后再记录构图、画质和交付可用性评分。精修建议采用“只改变 X；必须保持 Y”，并回到对话开始下一轮单点修改。

工作流知识来自 [OpenAI Cookbook](https://github.com/openai/openai-cookbook)、[awesome-gpt-image-2](https://github.com/freestylefly/awesome-gpt-image-2) 和 [GPT-Image2-Skill](https://github.com/wuyoscar/GPT-Image2-Skill)。应用只内置稳定的分类、模板元数据和 Craft 规则，不镜像第三方完整案例，也不安装第三方 CLI；现有渠道、队列、计费、素材、动画和审计仍是唯一执行入口。`prompt-optimizer` 不在此工作流中。
...
测试覆盖账户与余额、工作站上限、聊天时间/耗时、附件隔离、AI 模板筛选、提示词一致性门槛、三阶段质量、图片 AI 验收、生成锁定、批量预占、取消退款、成功/失败结算、配置加密与版本冲突、热刷新和 30 天清理。领域划分、测试基类、fixture 和新增用例原则见 `tests/README.md`。
