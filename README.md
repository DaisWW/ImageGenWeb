...
- 聊天附件默认作为垫图；用户在生成前选择生图渠道，任务只使用所选渠道，不因满载、熔断或调用失败切换到其他渠道。
...
全局并发为 6。用户提交时选择刀哥的或 Lucen；任务同时受全局并发、所选渠道并发和单用户并发限制。所选渠道满载时任务继续排队，不会占用另一渠道。渠道优先级仍供 `channel_id` 为空的兼容请求自动调度使用；每张图片会记录实际使用的渠道，管理员可在生成记录和运行日志中查看。

明确选择渠道的任务在上游连接错误、超时、限流或可恢复 HTTP 错误后不会跨渠道重试；`channel_id` 为空的兼容自动调度请求仍会排除失败渠道并重新入队，默认最多尝试 2 个渠道。渠道在 120 秒内连续失败 3 次后熔断 300 秒，恢复时默认只允许 1 个探测任务；这些参数均可由管理员调整。自动重试期间保留最高候选价格的预占，最终仅按成功渠道的实际价格扣费；全部失败或取消时释放预占。超时或连接中断时无法确认上游是否已受理，自动切换可能产生平台侧重复上游成本，但不会向用户重复扣费。
