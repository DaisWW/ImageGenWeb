...
自动调度在上游连接错误、超时、限流或可恢复 HTTP 错误后会排除已失败渠道并重新入队，默认最多尝试 2 个渠道；旧 API 传入具体 `channel_id` 的请求仍保持固定渠道兼容行为。渠道在 120 秒内连续失败 3 次后熔断 300 秒，恢复时默认只允许 1 个探测任务；这些参数均可由管理员调整。自动重试期间保留最高候选价格的预占，最终仅按成功渠道的实际价格扣费；全部失败或取消时释放预占。超时或连接中断时无法确认上游是否已受理，自动切换可能产生平台侧重复上游成本，但不会向用户重复扣费。
