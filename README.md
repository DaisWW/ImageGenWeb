...
- 聊天附件默认作为垫图；用户无需选择生图渠道，系统按渠道优先级自动调度，首选渠道达到并发上限后自动使用下一渠道。
...
全局并发为 6。系统按优先级从小到大分配；当刀哥的渠道已占满 2 个槽位时，后续任务会落到 Lucen，实际调度同时受全局并发、各渠道并发和单用户并发限制。每张图片会记录实际使用的渠道，管理员可在生成记录和运行日志中查看。

上游发生连接错误、超时、限流或可恢复 HTTP 错误时，同一张图片会排除失败渠道并重新入队，默认最多尝试 2 个渠道。渠道在 120 秒内连续失败 3 次后熔断 300 秒，恢复时默认只允许 1 个探测任务；这些参数均可由管理员调整。重试期间保留最高候选价格的预占，最终仅按成功渠道的实际价格扣费；全部失败或取消时释放预占。超时或连接中断时无法确认上游是否已受理，自动切换可能产生平台侧重复上游成本，但不会向用户重复扣费。
