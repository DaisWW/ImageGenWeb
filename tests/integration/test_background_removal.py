...
def transparent_candidate(color: tuple[int, int, int]) -> bytes:
    image = Image.new("RGBA", (72, 56), (0, 0, 0, 0))
    draw = ImageDraw.Draw(image)
    draw.rounded_rectangle((9, 7, 63, 49), radius=8, fill=(*color, 255))
    output = io.BytesIO()
...
class FakeMattingClient:
    ...
    def remove_background(self, content: bytes, *, filename: str) -> bytes:
        ...
        return transparent_candidate(color)
...
class TestBackgroundRemoval(PlatformTestCase):
    ...
    def _process_result(self, result_id: str):
        ...
        with patch("imagegen.worker.LucidaMattingClient", FakeMattingClient):
