...
const BACKGROUND_REMOVAL_ADAPTER_LABEL = {
  lucida: "Lucida HTTP",
  ...
  Object.assign(StudioApp.prototype, {
    ...
    renderBackgroundRemovalModels(selectedIds = null) {
      ...
      this.el.backgroundRemovalModelList.replaceChildren(
        ...this.backgroundRemovalModels.map((model) => {
          ...
          upstream.textContent = `${BACKGROUND_REMOVAL_ADAPTER_LABEL[model.adapter_id] || model.adapter_id || "Lucida HTTP"} · ${model.model}`;
    ...
    renderBackgroundRemovalRun() {
      ...
      this.el.backgroundRemovalResultList.innerHTML = results.map((result) => {
        ...
        const adapterLabel = BACKGROUND_REMOVAL_ADAPTER_LABEL[result.adapter_id] || result.adapter_id || "Lucida HTTP";
        ...
        return `<article class="background-removal-result ${UI.escapeHtml(result.status)}${isActive ? " active" : ""}${result.selected ? " selected" : ""}">
          ...
          <span class="background-removal-result-copy"><strong>${UI.escapeHtml(result.model_label)}</strong><small>${UI.escapeHtml(`${adapterLabel} · ${detail}`)}</small></span>
          </button>
