(() => {
  ...
  class AdminApp {
    ...
    renderChannels() {
      ...
      this.el.channelTableBody.innerHTML = config.channels.map((channel, index) => `
        <tr draggable="true" data-channel-index="${index}">
          ...
          <td><div class="capability-list"><span>${channel.capabilities.modes.includes("img2img") ? `最多 ${channel.capabilities.max_reference_images} 张垫图` : "仅文生图"}</span><span>${channel.capabilities.supports_mask ? "支持局部重绘" : "未启用蒙版"}</span><span>${channel.capabilities.formats.join(" / ")}</span></div></td>
          <td class="actions-cell"><div class="row-actions">
    ...
    channelFromForm() {
      ...
      return {
        ...
        capabilities: {
          ...
          supports_mask: form.elements.supports_mask.checked,
    ...
    renderMattingModels() {
      ...
      this.el.mattingModelTableBody.innerHTML = config.models.map((model) => `
        <tr>
          ...
          <td>${UI.escapeHtml(MATTING_ADAPTER_LABEL[model.adapter_id] || model.adapter_id || "Lucida HTTP")}<small class="subline">${UI.escapeHtml(model.model)} · ${UI.escapeHtml(model.base_url || "本地处理")}</small></td>
    ...
    openMattingModelDialog(identifier = "") {
      ...
      form.elements.label.value = model?.label || "";
      form.elements.enabled.checked = model ? model.enabled : true;
      form.elements.adapter_id.value = model?.adapter_id || model?.backend || "lucida";
      form.elements.base_url.value = model?.base_url || "";
      form.elements.model.value = model?.model || "";
      form.elements.options_json.value = model?.options ? JSON.stringify(model.options, null, 2) : "";
    ...
    async saveMattingModel(event) {
      ...
      try {
        const form = this.el.mattingModelForm;
        let options = {};
        const optionsText = form.elements.options_json.value.trim();
        if (optionsText) {
          options = JSON.parse(optionsText);
          if (!options || typeof options !== "object" || Array.isArray(options)) {
            throw new Error("适配器参数必须是 JSON 对象");
          }
        }
        const model = {
          id: form.elements.id.value.trim(),
          label: form.elements.label.value.trim(),
          enabled: form.elements.enabled.checked,
          adapter_id: form.elements.adapter_id.value,
          base_url: form.elements.base_url.value.trim(),
          model: form.elements.model.value.trim(),
          options,
          timeout_seconds: Number(form.elements.timeout_seconds.value),
