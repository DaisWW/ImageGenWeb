...
class RuntimeConfigService:
    ...
    def _matting_document(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for raw in raw_models:
            ...
            adapter_id = raw.get(
                "adapter_id",
                raw.get("adapter", raw.get("backend", old.adapter_id if old else "lucida")),
            )
            options = raw.get("options", copy.deepcopy(old.options) if old else {})
            models.append(
                {
                    "id": identifier,
                    "label": str(raw.get("label", "")).strip(),
                    ...
                    "base_url": str(raw.get("base_url", old.base_url if old else "")).strip(),
                    "model": str(raw.get("model", old.model if old else "")).strip(),
                    "timeout_seconds": raw.get(
                        "timeout_seconds", old.timeout_seconds if old else None
                    ),
                    "max_concurrency": raw.get(
