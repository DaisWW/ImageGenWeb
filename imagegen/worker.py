...
class GenerationWorker:
    def __init__(
        ...
        *,
        ...
        self.matting_adapters: MattingAdapterFactory = self.background_removal.adapter_factory
    ...
    def _process_background_removal(self, result_id: str) -> None:
        ...
        with self.app.app_context():
            ...
            try:
                ...
                adapter = self.background_removal.create_adapter(result)
                content = adapter.remove_background(
                    source,
                    filename=f"image_{item.id}.{item.job.output_format or 'png'}",
                )
                expected_size = (
                    (item.output_width, item.output_height)
                    if item.output_width and item.output_height
                    else None
                )
                validate_matting_output(content, expected_size=expected_size)
