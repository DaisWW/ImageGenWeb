...
若用户从外部图库复制提示词，只提取可复用结构，以当前需求覆盖案例中的主体、文字、品牌和参考图职责。"""
    ...
    routed_direction_ids = {template.direction_id for template in candidates}
    rule_directions = (
        tuple(item for item in directions if item.identifier in routed_direction_ids)
        if candidates
        else directions
    )
    selected_templates = candidates or tuple(
        template
...
游戏方向特别规则：game_ui 与 game_art 必须分开。game_ui 先锁定平台、目标画布、屏幕状态和安全区，再定义 HUD/组件分区；game_art 先锁交付阶段、身份锚点、镜头动作、面板职责和跨面板一致性。未被系统检索出的案例编号只代表结构范围；系统随后提供的案例摘录也只能用于结构参考，不能复制其中的 IP、角色、Logo 或文字。
...
{template_heading}（融合 awesome-gpt-image-2 与 GPT-Image2-Skill 的结构参考）：
{template_rows}

{gallery_heading}（完整 Atlas 共 31 类、162 个 Case；以下是类别结构摘要）：
{gallery_categories}
...
selection_reason 用一句中文说明为什么该方向/模板最符合交付物，不得声称读取过系统未提供的 Case 内容。
