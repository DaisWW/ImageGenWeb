...
- 用户提交生图时必须选择具体渠道：服务端校验并预占该渠道，将每个 `GenerationItem` 固定到所选渠道；公开渠道接口返回当前占用、可用槽位和并发上限，满载提交直接拒绝。调用失败只结束当前所选渠道，不自动切换或重新排队；超时或连接中断标记为中断并释放预占。用户并发限制按账户汇总；每个条目保存所选渠道、名称和价格，管理员记录与运行日志以条目值为准。
