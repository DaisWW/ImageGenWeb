...
- 生成计划在提交请求内构造，不新增临时表：每个 `GenerationItem` 持久化最终实际提示词，`sample`、`explore` 和 `series` 的策略与契约只作为任务工作流元数据保存，便于重试、验收和审计。
 - 用户提交生图时选择具体渠道：服务端只校验并预占该渠道，将每个 `GenerationItem` 固定到所选渠道；满载或熔断时继续等待，调用失败时不跨渠道重试。`channel_id` 为空的兼容请求仍按 `priority`（数值越小越优先）自动分配，并可在可恢复错误后尝试其他候选。用户并发限制按账户汇总；每个条目保存实际渠道、名称、价格和尝试记录，管理员记录与运行日志以条目值为准。
