...
- 用户提交生图时选择具体渠道，服务端只校验并预占该渠道，将每个 `GenerationItem` 固定到所选渠道；满载时继续排队，调用失败时不跨渠道重试。账户不设生图并发或排队上限。旧 API `channel_id` 为空的请求仍按渠道配置列表顺序自动分配，并可在可恢复错误后尝试其他候选。每个条目保存实际渠道、名称、价格和尝试记录，管理员记录与运行日志以条目值为准。
- 图片尺寸不按渠道维护白名单；服务端按 GPT Image 2 系列规则校验 `auto` 或 16 倍数自定义尺寸、比例、像素数和最大边，实际返回尺寸记录在生成条目中。旧工作站尺寸在下次设置更新时归一为 `auto`，历史任务不改写。

## 扩展方式

新增图片渠道时，优先在 `imagegen/integrations/` 实现适配器，并在 `ProviderFactory` 中注册。渠道能力、价格、列表顺序、模型和并发属于管理员配置，不应写进路由。
