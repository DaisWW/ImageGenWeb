...
当前部署契约是单 Gunicorn Web 进程、单 Worker。登录限流和工作站变更互斥保存在 Web 进程内；Worker 使用数据库租约拒绝第二个活跃实例。需要水平扩容前，必须先把这些协调状态迁移到共享存储，不能直接增加进程或副本数。
...
imagegen/
  ...
  config/             已校验、支持热刷新的渠道、聊天与透明化配置
  integrations/       生图/聊天 HTTP 客户端和背景透明化适配器
...
- 生图提交完成后统一进入队列；Worker 只根据渠道并发槽位、熔断和重试状态调度。用户名由数据库函数索引保证大小写不敏感唯一。
- 图片文件先写入，再提交对应数据库记录；回滚时删除文件。
- 删除与保留期清理先删除文件，再提交元数据删除；失败时保留元数据供下一次重试。
- 运行配置作为带版本文档保存，并使用旧值做原子版本校验。
- 生成计划在提交请求内构造，不新增临时表：每个 `GenerationItem` 持久化最终实际提示词，`sample` 和 `explore` 的策略与契约只作为任务工作流元数据保存，便于重试和审计。
- 用户提交生图时选择具体渠道，服务端只校验并预占该渠道，将每个 `GenerationItem` 固定到所选渠道；满载时继续排队，调用失败时不跨渠道重试。账户不设生图并发或排队上限。旧 API `channel_id` 为空的请求仍按 `priority`（数值越小越优先）自动分配，并可在可恢复错误后尝试其他候选。每个条目保存实际渠道、名称、价格和尝试记录，管理员记录与运行日志以条目值为准。
- 图片尺寸不按渠道维护白名单；服务端按 GPT Image 2 系列规则校验 `auto` 或 16 倍数自定义尺寸、比例、像素数和最大边，实际返回尺寸记录在生成条目中。旧工作站尺寸在下次设置更新时归一为 `auto`，历史任务不改写。

## 扩展方式

新增图片渠道时，优先在 `imagegen/integrations/` 实现适配器，并在 `ProviderFactory` 中注册。渠道能力、价格、优先级、模型和并发属于管理员配置，不应写进路由。
