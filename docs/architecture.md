...
- 生图提交不绑定用户渠道：服务端收集当前请求可用的渠道并按 `priority`（数值越小越优先）排序，预占候选渠道中的最高单价；Worker 在每个渠道的并发槽位上按优先级分配，首选渠道满后继续使用后续渠道。用户并发限制按账户汇总，跨渠道仍共享同一上限；每个 `GenerationItem` 保存实际渠道、名称和价格，管理员记录与运行日志以条目值为准。
