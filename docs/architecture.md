...
- 用户提交生图时选择具体渠道：服务端只校验并预占该渠道，将每个 `GenerationItem` 固定到所选渠道；满载或熔断时继续等待，调用失败时也不跨渠道重试。`channel_id` 为空的兼容请求仍收集可用渠道并按 `priority`（数值越小越优先）自动分配，首选渠道满后可使用后续渠道，并在可恢复错误后尝试其他候选。用户并发限制按账户汇总；每个条目保存实际渠道、名称和价格，管理员记录与运行日志以条目值为准。
