...
- 用户提交生图后由服务端按 `priority`（数值越小越优先）自动分配，首选渠道满载、熔断或发生可恢复错误时使用后续候选；旧 API 传入具体 `channel_id` 的请求保留固定渠道兼容行为。服务端按最高候选价格预占，用户并发限制按账户汇总；每个条目保存实际渠道、名称、价格和尝试记录，管理员记录与运行日志以条目值为准。
