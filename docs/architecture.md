...
- 用户提交生图时选择具体渠道，服务端只校验并预占该渠道，将每个 `GenerationItem` 固定到所选渠道；满载时继续排队，调用失败时不跨渠道重试。旧 API `channel_id` 为空的请求仍按 `priority`（数值越小越优先）自动分配，并可在可恢复错误后尝试其他候选。用户并发限制按账户汇总；每个条目保存实际渠道、名称、价格和尝试记录，管理员记录与运行日志以条目值为准。
